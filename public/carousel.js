...
function generateCarouselHtml(images) {
    ...
    if (typeof window.generateSwiperCarouselHtml === 'function') {
        console.log("Using enhanced Swiper carousel generator");
        return window.generateSwiperCarouselHtml(images);
    }
    ...
    const processedImages = images.map(image => {
        ...
        // Process URL through enhancedImageProcessor if available
        const imgUrl = typeof enhancedImageProcessor === 'function' ? 
            enhancedImageProcessor(url).thumbnail : url;
        
        return { url: imgUrl, caption: caption, originalUrl: url };
    });
    
    // Create HTML for carousel
    let html = `
    <div id="${carouselId}" class="simple-carousel relative mx-auto my-4 max-w-5xl">
    ...
    processedImages.forEach((image, index) => {
        html += `
                <div class="carousel-slide min-w-full sm:min-w-[calc(50%-10px)] lg:min-w-[calc(33.333%-10px)] px-1">
                    <div class="relative bg-white dark:bg-gray-700 rounded-lg overflow-hidden shadow-lg h-full">
                        <div class="image-container">
                            <img src="${image.url}" 
                                alt="${image.caption}" 
                                class="adaptive-image" 
                                data-original-url="${image.originalUrl}"
                                onclick="openLightbox('${image.url}', '${image.originalUrl}')"
                                onerror="this.classList.add('error-image'); if (window.setupGoogleDriveImageFallback) window.setupGoogleDriveImageFallback(this, this.getAttribute('data-original-url') || this.src);">
                        </div>
                        ${image.caption ? `<div class="p-3 text-center text-gray-700 dark:text-gray-200">${image.caption}</div>` : ''}
                    </div>
                </div>`;
    });
    
    // Add navigation, dots, etc.
    html += `
            </div>
        </div>
        
        <!-- Navigation Buttons -->
        <button class="prev-btn absolute left-2 top-1/2 transform -translate-y-1/2 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 w-10 h-10 rounded-full shadow-lg z-10 flex items-center justify-center">
            <i class="fas fa-chevron-left"></i>
        </button>
        <button class="next-btn absolute right-2 top-1/2 transform -translate-y-1/2 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 w-10 h-10 rounded-full shadow-lg z-10 flex items-center justify-center">
    ...
    processedImages.forEach((_, index) => {
        html += `<span class="carousel-dot w-3 h-3 mx-1 rounded-full bg-gray-300 dark:bg-gray-600 cursor-pointer ${index === 0 ? 'active' : ''}"></span>`;
    });
